 ...
 * Gebruikt CONFIG.MAX_STEP_RETRIES voor het maximum aantal pogingen.
 */
...
// ============================================================================
/**
 * Centraal register van alle DOM selectors.
 * Bij website wijzigingen: update hier, niet in 350+ plekken!
 */
const SELECTORS = {
  // Navigation
  continueButton: 'input[value="Volgende"]',
  continueTabButton: '#btnVolgendeTab',
  btn12: '#btn12',
  nextButton: '#FWS_Object\\.0\\.FWS_Objectlokatie\\.0\\.next',

  // Personal info
  bsn: '#link_aanv\\.0\\.link_aanv_persoon\\.0\\.edBSNnummer',
  initials: '#link_aanv\\.0\\.link_aanv_persoon\\.0\\.edVoorletters2',
  lastName: '#link_aanv\\.0\\.link_aanv_persoon\\.0\\.edAchternaam2',
  genderMale: '#link_aanv\\.0\\.link_aanv_persoon\\.0\\.choice_Geslacht_M',
  genderFemale: '#link_aanv\\.0\\.link_aanv_persoon\\.0\\.choice_Geslacht_V',

  // Contact
  phone: '#link_aanv\\.0\\.link_aanv_persoon\\.0\\.link_aanv_persoon_telefoon\\.0\\.edTelefoonField3',
  email: '#link_aanv\\.0\\.link_aanv_persoon\\.0\\.link_aanv_persoon_email\\.0\\.edEmailField3',
  iban: '#link_aanv\\.0\\.edIBAN',

  // Address
  postalCode: '#link_aanv\\.0\\.link_aanv_adres_vst\\.0\\.edPostcode',
  houseNumber: '#link_aanv\\.0\\.link_aanv_adres_vst\\.0\\.edHuisnummer2',
  houseNumberAddition: '#link_aanv\\.0\\.link_aanv_adres_vst\\.0\\.edToevoeging2',
  addressDifferent: '#FWS_Object\\.0\\.FWS_Objectlokatie\\.0\\.Adresafwijkend_J',

  // Installation
  meldcodeLookup: '#FWS_Object\\.0\\.FWS_Objectlokatie\\.0\\.FWS_Objectlokatie_ISDEPA\\.0\\.FWS_ObjectLocatie_ISDEPA_Meldcode\\.0\\.lookup_meldcode',
  purchaseDate: '#FWS_Object\\.0\\.FWS_Objectlokatie\\.0\\.FWS_Objectlokatie_ISDEPA\\.0\\.FWS_ObjectLocatie_ISDEPA_Meldcode\\.0\\.DatumAangeschaft',
  installationDate: '#FWS_Object\\.0\\.FWS_Objectlokatie\\.0\\.FWS_Objectlokatie_ISDEPA\\.0\\.FWS_ObjectLocatie_ISDEPA_Meldcode\\.0\\.DatumInstallatie',
  warmtepompChoice: '#FWS_Object\\.0\\.FWS_Objectlokatie\\.0\\.FWS_Objectlokatie_ISDEPA\\.0\\.FWS_ObjectLocatie_ISDEPA_Meldcode\\.0\\.choice_warmtepomp',
  addMeasure: '#FWS_Object\\.0\\.FWS_Objectlokatie\\.0\\.FWS_Objectlokatie_ISDEPA\\.0\\.addInvestering',
  bagDifferentYes: '#FWS_Object\\.0\\.FWS_Objectlokatie\\.0\\.FWS_Objectlokatie_ISDEPA\\.0\\.BAGafwijkend_J',
  wizardNext: '#FWS_Object\\.0\\.FWS_Objectlokatie\\.0\\.FWS_Objectlokatie_ISDEPA\\.0\\.FWS_ObjectLocatie_ISDEPA_Meldcode\\.0\\.wizard_investering_volgende',

  // Company
  kvkNumber: '#FWS_Object\\.0\\.FWS_Objectlokatie\\.0\\.FWS_Objectlokatie_ISDEPA\\.0\\.FWS_ObjectLocatie_ISDEPA_Meldcode\\.0\\.InstallBedrijf_KvK',
  companyName: '#FWS_Object\\.0\\.FWS_Objectlokatie\\.0\\.FWS_Objectlokatie_ISDEPA\\.0\\.FWS_ObjectLocatie_ISDEPA_Meldcode\\.0\\.InstallBedrijf_Naam',
  dutchCompany: '#FWS_Object\\.0\\.FWS_Objectlokatie\\.0\\.FWS_Objectlokatie_ISDEPA\\.0\\.FWS_ObjectLocatie_ISDEPA_Meldcode\\.0\\.InstallBedrijf_NL_jn_J',

  // Intermediary
  extraContactPerson: '#link_int\\.0\\.link_int_organisatie\\.0\\.edExtraContactpersoon_n_int',
  digitalCorrespondence: '#link_int\\.0\\.edDigitaleCorrespondentie_J',

  // Documents
  fileUploadButton: '#FWS_Object\\.0\\.FWS_Objectlokatie\\.0\\.FWS_Objectlokatie_ISDEPA\\.0\\.FWS_ObjectLocatie_ISDEPA_Meldcode\\.0\\.Bijlagen_NogToevoegen_ISDEPA_Meldcode\\.0\\.btn_ToevoegenBijlage',
  fileInput: '#lip_modalWindow div.content input[type="file"], #lip_attachments_resumable input[type="file"]',
  ...
  // Modals
  lipModalWindow: '#lip_modalWindow',
  lipMatchcode: '#lip_matchcode',
  ...
  detectedStep: 'detected-step',
  stopButton: 'stop-automation',
  toggleConfigData: 'toggle-config-data',
  toggleIcon: 'toggle-icon',
  configDataContainer: 'config-data-container',
  configDataContent: 'config-data-content',
};
...
function createStatusPanel() {
  ...
  const existingPanel = document.getElementById(SELECTORS.automationPanel);
  if (existingPanel) {
  ...
  document.getElementById(SELECTORS.toggleConfigData).addEventListener('click', () => {
    const container = document.getElementById(SELECTORS.configDataContainer);
    const icon = document.getElementById(SELECTORS.toggleIcon);
    const content = document.getElementById(SELECTORS.configDataContent);
  ...
  document.getElementById(SELECTORS.stopButton).addEventListener('click', (e) => {
    ...
    const panelToRemove = document.getElementById(SELECTORS.automationPanel);
    if (panelToRemove) {
    ...
    setTimeout(() => {
      const finalPanel = document.getElementById(SELECTORS.automationPanel);
      if (finalPanel) {
        finalPanel.remove();
...
function updateStatus(message, step, detectedStep) {
  const statusDiv = document.getElementById(SELECTORS.automationStatus);
  const stepDiv = document.getElementById(SELECTORS.currentStep);
  const detectedDiv = document.getElementById(SELECTORS.detectedStep);
 ...
 *
 * BELANGRIJK: Ontvangt PRE-SANITIZED data van sanitization.js via popup.js
 * Alle OCR correcties en validaties zijn al gedaan voordat data hier aankomt.
 *
 * @param {string} selector - CSS selector van het invoerveld
 * @param {string} value - Pre-sanitized waarde om in te vullen
 */
async function fillInput(selector, value) {
  ...
  // Vul de pre-sanitized waarde in
  element.value = value;
  element.dispatchEvent(new Event('input', { bubbles: true }));
...
async function uploadFile(fileData) {
  ...
  try {
    ...
    const modal = document.querySelector(SELECTORS.lipModalWindow);
    if (modal) {
...
function fillCurrentPage(config) {
  ...
  const fields = [
    [SELECTORS.bsn, config.bsn],
    [SELECTORS.initials, config.initials],
    [SELECTORS.lastName, config.lastName],
    [SELECTORS.phone, config.phone],
    [SELECTORS.email, config.email],
    [SELECTORS.iban, config.iban],
    [SELECTORS.postalCode, config.postalCode],
    [SELECTORS.houseNumber, config.houseNumber],
    [SELECTORS.houseNumberAddition, config.houseNumberAddition],
    [SELECTORS.companyName, config.companyName],
    [SELECTORS.kvkNumber, config.kvkNumber]
  ];
...
function detectCurrentStep() {
  ...
  if (document.querySelector(SELECTORS.akkoord) && document.querySelector(SELECTORS.submitButton)) {
  ...
  if ((hasVoorlopigSubsidiebedrag || hasAangevraagdeMaatregelen) && document.querySelector(SELECTORS.continueTabButton)) {
  ...
  const hasFileUploadButton = document.querySelector(SELECTORS.fileUploadButton);
  ...
  if (document.querySelector(SELECTORS.purchaseDate) &&
      document.querySelector(SELECTORS.installationDate) &&
  ...
  if (document.querySelector(SELECTORS.bagDifferentYes)) {
    // Additional check: make sure we're not on the "add measure" page
    const hasAddMeasureButton = document.querySelector(SELECTORS.addMeasure);
  ...
  const noBagRadio = !document.querySelector(SELECTORS.bagDifferentYes);
  ...
  if (document.querySelector(SELECTORS.addressDifferent)) {
  ...
  if (document.querySelector(SELECTORS.extraContactPerson) &&
      document.querySelector(SELECTORS.digitalCorrespondence)) {
  ...
  if (document.querySelector(SELECTORS.bsn) &&
      document.querySelector(SELECTORS.initials)) {
    console.log('🎯 Detected: info_acknowledged - Personal details page');
    return 'info_acknowledged';
  }

  // Step 5: Info acknowledgment checkbox
  if (document.querySelector(SELECTORS.infoRead)) {
  ...
  const catalogLink = document.querySelector(SELECTORS.catalogLink);
  ...
  const hasNaarWaarheidCheckbox = document.querySelector(SELECTORS.naarWaarheid);
  ...
  const startLinkById = document.querySelector(SELECTORS.startLink);
 ...
 * - Stopt automatisch na CONFIG.MAX_STEP_RETRIES pogingen
...
async function startFullAutomation(config) {
  ...
  try {
    ...
    if (currentStep === lastExecutedStep) {
      ...
      if (stepExecutionCount >= CONFIG.MAX_STEP_RETRIES) {
    ...
    if (currentStep === 'start') {
      ...
      let startLink = document.querySelector(SELECTORS.startLink);
      if (!startLink) {
    ...
    if (currentStep === 'nieuwe_aanvraag_clicked' && document.querySelector(SELECTORS.btn12)) {
    ...
    const isActuallyIntroPage = !document.querySelector(SELECTORS.naarWaarheid);
    ...
    if (document.querySelector(SELECTORS.btn12) && (currentStep === 'isde_selected' || detectedStep === 'isde_selected')) {
      ...
      await clickElement(SELECTORS.btn12);
      return;
    }
    ...
    if (document.querySelector(SELECTORS.naarWaarheid) && (currentStep === 'first_volgende_clicked' || currentStep === 'intro_role_completed' || currentStep === 'intro_role_selection')) {
      ...
      await ensureChecked(SELECTORS.naarWaarheid);
    ...
    if (document.querySelector(SELECTORS.infoRead) &&
      ...
      await ensureChecked(SELECTORS.infoRead);
      await clickElement(SELECTORS.continueTabButton);
    ...
    if (document.querySelector(SELECTORS.bsn) &&
      ...
      await fillInput(SELECTORS.phone, config.phone);
      await unthrottledDelay(800 + Math.random() * 400); // Mensachtig - robot detectie preventie

      await fillInput(SELECTORS.email, config.email);
      await unthrottledDelay(900 + Math.random() * 500); // Mensachtig - robot detectie preventie

      await fillInput(SELECTORS.iban, config.iban);
      await unthrottledDelay(3000); // Extra lange pauze voor IBAN validatie

      await fillInput(SELECTORS.postalCode, config.postalCode);
      await unthrottledDelay(700 + Math.random() * 300); // Mensachtig - robot detectie preventie

      await fillInput(SELECTORS.houseNumber, config.houseNumber);
      await unthrottledDelay(600 + Math.random() * 300); // Mensachtig - robot detectie preventie

      await fillInput(SELECTORS.houseNumberAddition, config.houseAddition);
    ...
    if (currentStep === 'correspondence_done') {
      ...
      const addressDifferentCheckbox = document.querySelector(SELECTORS.addressDifferent);
      ...
      if (addressDifferentCheckbox) {
        ...
        const volgendeButton = document.querySelector('input[value="Volgende"]') || 
                              document.querySelector(SELECTORS.continueTabButton);
        if (volgendeButton) {
          await clickElement(volgendeButton);
        }
      }
    ...
    if (document.querySelector(SELECTORS.bagDifferentYes) &&
      ...
      const volgendeTab = document.querySelector(SELECTORS.continueTabButton) ||
    ...
    if (currentStep === 'installation_address_confirmation' ||
      ...
      if (detectedStep === 'installation_address_confirmation') {
        ...
        if (!volgendeButton) {
          volgendeButton = document.querySelector(SELECTORS.continueTabButton);
        }
    ...
    if (currentStep === 'meldcode_search_in_wizard' || detectedStep === 'meldcode_search_in_wizard') {
      ...
      const searchInput = document.querySelector(SELECTORS.lipMatchcode) ||
      ...
      if (searchInput && config.meldCode) {
        ...
        const volgendeBtn = document.querySelector(SELECTORS.wizardNext) ||
                           document.querySelector('input[value="Volgende"]');
    ...
    if (document.querySelector(SELECTORS.purchaseDate) &&
      ...
      const dutchCompanyRadio = document.querySelector(SELECTORS.dutchCompany);
      ...
      const kvkField = document.querySelector(SELECTORS.kvkNumber);
      if (kvkField) {
    ...
    if ((currentStep === 'installation_details_done' || detectedStep === 'installation_details_done') &&
        document.querySelector(SELECTORS.wizardNext)) {
    ...
    if (currentStep === 'date_continued') {
      ...
      const lookupButton = document.querySelector(SELECTORS.meldcodeLookup);
      if (lookupButton) {
    ...
    if ((currentStep === 'meldcode_selected' || currentStep === 'meldcode_clicked' || currentStep === 'measure_confirmed') &&
      ...
      let volgendeButton = document.querySelector(SELECTORS.wizardNext);
    ...
    if ((currentStep === 'measure_overview' || detectedStep === 'measure_overview') &&
      ...
      let volgendeButton = document.querySelector(SELECTORS.continueTabButton);
      ...
      if (!volgendeButton) {
        ...
        volgendeButton = document.querySelector(SELECTORS.wizardNext);
      }
...
(async () => {
  while (!automationStopped) {
    ...
    const automationConfig = sessionStorage.getItem('automationConfig');
    if (automationConfig && !document.getElementById(SELECTORS.automationPanel)) {
